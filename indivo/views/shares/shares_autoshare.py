...
def autoshare_list(request, record):
  ...
  if request.GET.has_key(TYPE):
    try:
      docschema = DocumentSchema.objects.get(type = request.GET[TYPE])
...
def autoshare_create(request, record, carenet):
  ...
  if request.POST.has_key(TYPE):
    try:
      docschema = DocumentSchema.objects.get(type = request.POST[TYPE])
    except DocumentSchema.DoesNotExist:
      raise Http404
...
def autoshare_revert(request, record, carenet):
  # Not yet implemented
  return DONE
