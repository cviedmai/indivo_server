...
def request_token_approve(request, request_token):
  ...
  request_token = OAUTH_SERVER.authorize_request_token(rt.token, record = record, carenet = carenet, account = request.principal)
...
# a decorator
def app_specific_perm_check(func):
  def _check_app_specific_perm(request, record_id, pha_email, *args, **kwargs):
    # check it against pha_id
    try:
      pha, token, params = get_pha_info(request)

      assert pha.email        == pha_email, "correct consumer"
      assert token.record.id  == record_id, "correct record"

      # check that the token can access the record
      assert token.record.canAccess(token), "correct token"
    except:
      raise PermissionDenied()

    return func(request, token.record, pha, *args, **kwargs)
    
  _check_app_specific_perm.__doc__ = func.__doc__
  _check_app_specific_perm.__dict__ = func.__dict__
  return _check_app_specific_perm

@app_specific_perm_check
def pha_document_update(request, record, pha, document_id):
  try:
    doc = Document.objects.get(id = document_id, record = record, pha = pha)
